...
namespace PhysicsCharacterController
{
    ...
    public class CharacterManager : MonoBehaviour
    {
        ...
        public float vaultLockTime = 0.3f;
        ...
        [Header("Wall Jump Rules")]
        public float wallJumpControlLockTime = 0.22f;

        // -0.7 = bastante enfrentada.
        // -0.85 = muy estricta.
        // -0.5 = más permisiva.
        public float oppositeWallThreshold = -0.7f;

        // --- NUEVO: memoria del último wall jump ---
        private Vector3 lastWallJumpNormal;
        private bool hasWallJumped = false;

        // --- NUEVO: tiempo durante el cual no controlas el aire tras rebotar ---
        ...
        private float ignoreGroundTimer;
        ...
        private void FixedUpdate()
        {
            ...
            // --- NUEVO: timer del bloqueo de control tras wall jump ---
        ...
        private void CheckGrounded()
        {
            ...
            isGrounded = Physics.CheckSphere(
                transform.position - new Vector3(0, originalColliderHeight / 2f, 0),
                groundCheckerThrashold,
                groundMask
            );
        }
        ...
        public void PoleVault(Vector3 direction, float force, float upForce)
        {
            Vector3 vel = rigidbody.linearVelocity;

            // mantenemos vertical limpia
            vel.y = 0;
            rigidbody.linearVelocity = vel;

            // impulso horizontal + vertical controlado
        ...
        public void IgnoreGroundFor(float time)
        {
            ignoreGroundTimer = time;
        }
        private void MoveWalk()
        {
            ...
            // --- NUEVO: durante el rebote de pared, el jugador NO controla el aire ---
            // Esto evita que pueda cancelar el impulso y volver a pegarse a la misma pared.
            if (wallJumpControlLockTimer > 0f)
            {
                return;
            }
            ...
            {
                targetAngle =
                    Mathf.Atan2(axisInput.x, axisInput.y) *
                    Mathf.Rad2Deg +
            ...
            Vector3 smoothedHorizontal =
                Vector3.SmoothDamp(
                    horizontalVel,
                    new Vector3(targetVel.x, 0f, targetVel.z),
                    ref currVelocity,
                    axisInput.magnitude > movementThrashold
                        ? dampSpeedUp
                        : dampSpeedDown
                );
        ...
        {
            //jumped
            if (jump && !isScaling && isGrounded && ((isTouchingSlope && currentSurfaceAngle <= maxClimbableSlopeAngle) || !isTouchingSlope) && !isTouchingWall)
            {
            ...
            {
                // --- NUEVO: comprobamos si esta pared es válida ---
                ...
                if (hasWallJumped)
                {
                    float dot = Vector3.Dot(
                        ...
                        lastWallJumpNormal.normalized
                    );

                    bool isOppositeWall = dot < oppositeWallThreshold;

                    // Si no está enfrentada, no dejamos saltar.
                    // Esto bloquea misma pared y esquinas interiores.
                    if (!isOppositeWall)
                    {
                        canWallJump = false;
                    }
                ...
                if (canWallJump)
                {
                    // --- NUEVO: dirección horizontal obligatoria hacia fuera de la pared ---
                    ...
                    // Seguridad por si la normal viene rara.
                    if (wallJumpDirection.sqrMagnitude < 0.001f)
                    {
                        wallJumpDirection = -transform.forward;
                    }

                    // --- MODIFICADO: en vez de sumar velocidad, la FORZAMOS ---
                    // Así el jugador no conserva velocidad previa que pueda romper el rebote.
                    Vector3 newVelocity =
                        wallJumpDirection * jumpFromWallMultiplier +
                    ...
                    targetAngle = Mathf.Atan2(wallJumpDirection.x, wallJumpDirection.z) * Mathf.Rad2Deg;

                    forward = wallJumpDirection;
                    globalForward = forward;
                    reactionForward = forward;

                    // --- NUEVO: guardamos esta pared como última pared usada ---
                    lastWallJumpNormal = wallNormal.normalized;
                    hasWallJumped = true;

                    // --- NUEVO: bloqueamos control aéreo un momentito ---
            ...
            // --- NUEVO: al tocar suelo, reseteamos la cadena de wall jumps ---
            if (isGrounded)
            {
                hasWallJumped = false;
        ...
        public bool CanStartPoleVault()
        {
            return isGrounded && !isVaulting && !isScaling;
        }

        public void SetVaultingState(bool value)
        {
            isVaulting = value;
